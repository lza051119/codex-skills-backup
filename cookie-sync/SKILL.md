...
# Cookie Sync 鈥?Local Chrome 鈫?Browserbase Context
...
```bash
cd "$HOME/.codex/skills/cookie-sync" && npm install
```

## Usage

### Basic 鈥?sync all cookies

```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs"
```

Creates a persistent context with all your Chrome cookies. Outputs a context ID.

### Filter by domain 鈥?only sync specific sites

```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --domains google.com,github.com
```
...
```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --context ctx_abc123
```
...
```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --stealth
```
...
```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --proxy "San Francisco,CA,US"
```
...
```bash
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --domains github.com,google.com --stealth --proxy "San Francisco,CA,US"
```
...
node "$HOME/.codex/skills/cookie-sync/scripts/cookie-sync.mjs" --domains x.com,twitter.com
...
1. **Once (laptop open):** Run cookie-sync 鈫?get a context ID
2. **Scheduled jobs:** Use `browse open <url> --context-id <ctx-id> --persist` 鈥?no local Chrome needed
3. **Re-sync as needed:** When cookies expire, run cookie-sync again with `--context <ctx-id>` to refresh

## Troubleshooting

- **"No DevToolsActivePort found"** 鈫?Enable `chrome://flags/#allow-remote-debugging` if your browser build exposes it, or launch with `--remote-debugging-port=9222` and set `CDP_URL=ws://127.0.0.1:9222`
- **"No open page targets found"** 鈫?Open at least one tab in Chrome
- **"WebSocket error"** 鈫?Chrome may be hung; force quit and reopen it
- **Cookies expired in context** 鈫?Re-run cookie-sync with `--context <id>` to refresh
- **Auth rejected by site** 鈫?Try adding `--stealth` and/or `--proxy` with a location near you
