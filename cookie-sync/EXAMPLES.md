...
This creates a new session using the saved context 鈥?no need to re-export cookies from Chrome (though the script still does to ensure freshness).
...
- **One context per identity** 鈥?don't mix personal and work browser cookies in the same context
- **Close sessions when done** 鈥?keepAlive sessions persist until explicitly closed and consume resources
- **Watch the live view** 鈥?open the viewer URL to see the cloud browser in real time
