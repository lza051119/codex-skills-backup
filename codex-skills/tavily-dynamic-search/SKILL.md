...
description: "Run programmatic multi-step Tavily searches from code. Use when search needs branching logic, iterative query generation, filtering, ranking, or custom aggregation before returning results."
---
...
This skill uses programmatic tool calling for web search: write code that orchestrates Tavily calls inside a sandbox, keep intermediate results out of the context window, and return only the final `print()` output.
