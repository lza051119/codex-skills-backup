...
Arguments:
- `<slug>` (required) 鈥?template name from `bb templates list`
- `[path]` (optional) 鈥?destination directory, defaults to the template slug

Options:
- `--language <language>` 鈥?`python` or `typescript`

## Browse passthrough

`bb browse ...` forwards arguments to the standalone `browse` binary (`@browserbasehq/browse-cli`). The examples below are `browse-cli` subcommands 鈥?they are not native `bb` commands:
...
Install Browserbase agent skills for Codex directly from the CLI:
