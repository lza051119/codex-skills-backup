...
description: Automate web browser interactions with the Browserbase browse CLI. Use for Browserbase-specific browsing, protected websites, remote browser sessions, persistent contexts, CAPTCHA/anti-bot workflows, screenshots, form filling, and JavaScript-heavy pages. For ordinary localhost testing, prefer Codex's built-in Browser Use or webapp-testing skills.
...
Automate browser interactions using the Browserbase `browse` CLI from Codex.
...
Use `browse snapshot` as your default for understanding page state 鈥?it returns the accessibility tree with element refs you can use to interact. Only use `browse screenshot` when you need visual context (layout, images, debugging).
...
1. `browse open <url>` 鈥?navigate to the page
2. `browse snapshot` 鈥?read the accessibility tree to understand page structure and get element refs
3. `browse click <ref>` / `browse type <text>` / `browse fill <selector> <value>` 鈥?interact using refs from snapshot
4. `browse snapshot` 鈥?confirm the action worked
5. Repeat 3-4 as needed
6. `browse stop` 鈥?close the browser when done
...
3. **Use `browse snapshot`** to check page state 鈥?it's fast and gives you element refs
4. **Only screenshot when visual context is needed** (layout checks, images, debugging)
5. **Use refs from snapshot** to click/interact 鈥?e.g., `browse click @0-5`
