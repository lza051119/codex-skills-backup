...
## Example 2: Form Validation 鈥?Happy Path, Errors, and Adversarial
...
# STEP_PASS|empty-submit|alert "Please enter a valid email" appeared 鈥?form handles empty input
...
# field 鈥?that's just the input value, not rendered HTML. The real checks are:
...
# Result: "<script>alert('xss')</script>" 鈥?payload stays as text in the input, not rendered as HTML
...
## Example 3: Modal Lifecycle 鈥?Full State Machine
...
# Tab through dialog 鈥?focus should stay inside
browse press Tab
...
# STEP_PASS|focus-trap|focus cycles within dialog (Cancel 鈫?Confirm 鈫?Cancel)
# or on failure:
browse screenshot --path .context/ui-test-screenshots/focus-trap.png
# STEP_FAIL|focus-trap|expected focus trapped in dialog 鈫?focus escaped to nav link behind modal|.context/ui-test-screenshots/focus-trap.png
...
# STEP_FAIL|axe-audit|expected 0 violations 鈫?found 2: color-contrast (serious, 3 nodes), label (critical, 1 node)
...
## Example 6: Console Health 鈥?Deterministic Error Detection
...
# STEP_FAIL|console-dashboard|expected 0 errors 鈫?1 error during interaction: "Failed to fetch /api/items"
...
```bash
# Step 1: Sync cookies
node .codex/skills/cookie-sync/scripts/cookie-sync.mjs --domains staging.myapp.com
...
## Example 8: Exploratory Testing 鈥?Try to Break It
...
# STEP_FAIL|empty-state|expected designed empty state 鈫?page is blank with no guidance|.context/ui-test-screenshots/empty-state.png
...
# or: STEP_FAIL|404-page|expected custom 404 鈫?got default Next.js error page
...
browse fill "textarea[name=message]" "<img src=x onerror=alert(1)> & \"quotes\" and emoji 馃帀"
...
```
FINDING: Contact form textarea has no maxlength 鈥?500+ char input accepted without truncation
SEVERITY: low
ROUTE: /contact
...
FINDING: No custom 404 page 鈥?default Next.js error shown
SEVERITY: medium
...
**User request**: "QA the staging site 鈥?test signup, dashboard, and accessibility in parallel."

The key insight: each test group gets its own `BROWSE_SESSION`, which spins up an independent Browserbase browser. Groups run concurrently via the Codex sub-agents.
...
Parallel Groups
===============
Group 1 (session: signup)     鈫?/signup form tests (happy path + adversarial)
Group 2 (session: dashboard)  鈫?/dashboard content + empty state
Group 3 (session: a11y)       鈫?/settings axe-core + keyboard nav
```
...
Use the Codex sub-agents 鈥?send all three in a single message so they run concurrently:
...
Agent 1 prompt:
  "You are running UI tests on https://staging.myapp.com/signup.
   ...
   For each test: snapshot BEFORE, act, snapshot AFTER, compare, emit:
     STEP_PASS|<id>|<evidence>  or  STEP_FAIL|<id>|<expected> 鈫?<actual>
...
Agent 2 prompt:
  "You are running UI tests on https://staging.myapp.com/dashboard.
   ...
   Run these tests:
   1. Check empty state 鈥?is there a message and CTA, or blank?
   2. Check data display 鈥?table columns, row count, formatting
   3. Check console errors 鈥?inject capture, interact, check __logs
...
Agent 3 prompt:
  "You are running accessibility tests on https://staging.myapp.com/settings.
   ...
   Run these tests:
   1. axe-core audit 鈥?load script, run, check violations
   2. Form labels 鈥?every input has an associated label
   3. Keyboard nav 鈥?Tab through all elements, verify focus order
   4. Broken images 鈥?check naturalWidth on all img elements
...
```
## UI Test Results (Parallel Run 鈥?3 Browserbase sessions)
...
STEP_FAIL|double-submit|expected single submission 鈫?two success toasts|.context/ui-test-screenshots/signup-double-submit.png
...
STEP_FAIL|axe-audit|expected 0 violations 鈫?2: color-contrast (serious, 3 nodes), label (critical, 1 node)|.context/ui-test-screenshots/a11y-axe-audit.png
...
```bash
# Safety net 鈥?stop any lingering sessions
...
- **Before/after for every interaction** 鈥?never assert without comparing state change
- **Deterministic checks are strongest** 鈥?axe-core count, console error array, overflow boolean
- **Try to break it** 鈥?empty input, long input, special chars, rapid clicks, keyboard-only
- **Use structured markers** 鈥?`STEP_PASS|id|evidence` or `STEP_FAIL|id|expected 鈫?actual|screenshot-path`
- **Screenshot every failure** 鈥?save to `.context/ui-test-screenshots/<step-id>.png` so devs can see what broke
- **Local for localhost** 鈥?never send localhost traffic through Browserbase
- **Default localhost run** 鈥?start with `browse env local` for clean, reproducible QA
- **Use `--auto-connect` selectively** 鈥?only when a localhost test explicitly needs existing local Chrome login/cookies/state (`browse env local --auto-connect`)
- **Explicit local CDP attach** 鈥?use `browse env local <port|url>` when you must target a specific local browser instance
- **Always `browse stop` when done** 鈥?for parallel runs, stop every named session
- **`.then()` not `await`** 鈥?browse eval doesn't support top-level await
- **Parallelize with `BROWSE_SESSION`** 鈥?each named session gets its own Browserbase browser; fan out via Codex sub-agents
