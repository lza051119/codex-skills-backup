...
Works with both local and remote mode. Named sessions are fully independent 鈥?each has its own browser process.
...
# They don't share state 鈥?each has its own page, cookies, refs
```

Local mode variants follow the CLI contract:

- `browse env local` 鈥?clean isolated local browser (default; preferred for reproducible localhost testing)
- `browse env local --auto-connect` 鈥?auto-discover local Chrome, fallback to isolated (use only when a test needs existing local login/cookies/state)
- `browse env local <port|url>` 鈥?explicit CDP attach to a specific local browser target

### When to use parallel vs sequential

| Scenario | Use |
|----------|-----|
| Tests on different pages/routes | **Parallel** 鈥?no shared state |
| Tests within one page (fill form 鈫?submit 鈫?check result) | **Sequential** 鈥?steps depend on each other |
| Accessibility audit + visual audit on same page | **Parallel** 鈥?independent checks |
| Before/after comparison on one element | **Sequential** 鈥?ordering matters |
...
=============================================
Group 1 (session: signup)     鈫?/signup form validation (happy + adversarial)
Group 2 (session: dashboard)  鈫?/dashboard empty state + data display
Group 3 (session: a11y)       鈫?/settings accessibility audit (axe-core + keyboard)
```
...
Use Codex sub-agents only when the user explicitly asks for parallel agent work. Each agent gets a unique session name and runs its test group independently:

```
Launch agents in parallel with Codex sub-agents:

Agent 1 鈥?prompt: "Run signup form tests using BROWSE_SESSION=signup.
...
Agent 2 鈥?prompt: "Run dashboard tests using BROWSE_SESSION=dashboard.
  Use `browse env local` first (localhost URL). Run these tests: [list tests].
  Follow the before/after assertion protocol.
  On any STEP_FAIL, immediately take a screenshot:
...
Agent 3 鈥?prompt: "Run accessibility audit using BROWSE_SESSION=a11y.
  Use `browse env local` first (localhost URL). Run these tests: [list tests].
  Follow the before/after assertion protocol.
  On any STEP_FAIL, immediately take a screenshot:
...
- Pass the full test steps and assertion protocol to each agent 鈥?they don't have the skill context
...
STEP_FAIL|double-submit|expected single submission 鈫?two success toasts appeared|.context/ui-test-screenshots/signup-double-submit.png
...
STEP_FAIL|axe-audit|expected 0 violations 鈫?2 critical: color-contrast, missing-label|.context/ui-test-screenshots/a11y-axe-audit.png
...
- signup-double-submit.png 鈥?duplicate toast after rapid submit
- a11y-axe-audit.png 鈥?page showing color contrast and missing label violations
```
...
```bash
# Sync once
node .codex/skills/cookie-sync/scripts/cookie-sync.mjs --domains staging.app.com
